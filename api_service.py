...
class APIService:
    """Train/cache the notebook's random-forest model and serve UI predictions."""
    def __init__(self):
        self.model = None
        self.features = None
    ...
    def _load_model(self):
        ...
        prep = ColumnTransformer([('numeric', MinMaxScaler(), numeric), ('categorical', OneHotEncoder(drop='first', handle_unknown='ignore'), categorical)])
        self.model = Pipeline([('features', prep), ('classifier', RandomForestClassifier(n_estimators=30, max_depth=10, random_state=42))])
        self.model.fit(X, y)
